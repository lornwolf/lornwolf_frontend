      ...
      reload(month, user = 'all') {
        ...
        return getMonthlyWorkTimes(month, user).then(response => {
          ...
          for(let i = 0; i < this.chartData.bars.length; i++) {
            this.chartData.bars[i].itemStyle.normal.label.formatter = function(p){return p.value > 0 ? p.value : '';};
          }
          this.initChart();
      ...
      initChart() {
        ...
        this.chart.setOption({
          ...
          legend: {
            ...
            data: ['管理', '会議・打合せ', '要件', '仕様', '製造', 'テスト（開発）', 'テスト（QA）', 'データ移行', '環境構築', '調査', 'レビュー', '待機', 'その他']
          },
          calculable: true,
          xAxis: [{
            type: 'category',
