<template>
  <v-app style="height: calc(100vh - 50px); overflow: hidden">
    ...
    <br>
    <br>
    <v-progress-linear v-model="totalProgress" />
    <br>
    <br>
      ...
      <v-card>
        ...
        <v-card-actions>
          ...
          <v-btn
            ...
            @click="
              dialog = false;
              $router.push({ path: '/study/calendar' });
            "
          >
            就此结束
          </v-btn>
...
export default {
  ...
  methods: {
    loadDictForStudy() {
      ...
      this.random = false;
      // 获取要复习的单词。
      fetch("http://localhost:8000/dictionary/select_words_for_study?loginId=lornwolf", { cache: "no-cache" })
        .then((res) => res.json())
        .then(async (json) => {
          if (json.length === 0) {
            ...
            try {
              let response = await fetch(
                "http://localhost:8000/dictionary/select_words_random?loginId=lornwolf&random=false",
                { cache: "no-cache" }
              );
      ...
      fetch("http://localhost:8000/dictionary/count_words_for_study?loginId=lornwolf", { cache: "no-cache" })
        .then((res) => res.text())
        .then((text) => {
          if (text && !isNaN(text)) {
    ...
    next(step = 1) {
      ...
      if (!currentWord.times) {
        ...
        if (!this.random) {
          ...
          if (nextResiveDate == null || nextResiveDate <= new Date()) {
            ...
            // 将下次复习时间更新至数据库。
            fetch("http://localhost:8000/dictionary/update_word?word=" + encodeURI(JSON.stringify(obj)), {
              cache: "no-cache",
            })
              .then((res) => res.text())
              .then((text) => {
        ...
        // 将最终复习时间更新至数据库。
        fetch("http://localhost:8000/dictionary/update_last_revise_date?id=" + currentWord.id + "&type=1", {
          cache: "no-cache",
        })
          .then((res) => res.text())
          .then((text) => {
      ...
      if (this.count === this.words.length * 2) {
        // 将复习记录保存至数据库。
        fetch("http://localhost:8000/dictionary/insert_revise_history?loginId=lornwolf&type=1", {
          cache: "no-cache",
        })
          .then((res) => res.text())
          .then((text) => {
    ...
    getDetail() {
      this.html = "";
      fetch("http://localhost:8000/dictionary/select_japanese_by_id?id=" + this.japaneseId + "&detail=true")
        .then((res) => res.json())
        .then((japanese) => {
              ...
              if (japanese.translations[j].examples && japanese.translations[j].examples.length > 0) {
                for (let k = 0; k < japanese.translations[j].examples.length; k++) {
                  this.html +=
                    ...
                    japanese.translations[j].examples[k].jp +
                    '&nbsp;&nbsp;<span style="color:royalblue">' +
                    japanese.translations[j].examples[k].cn +
                    "</span></span><br>";
                }
              }
            }
    ...
    randomWords() {
      ...
      this.random = false;
      // 获取要复习的单词。
      fetch("http://localhost:8000/dictionary/select_words_random?loginId=lornwolf&random=true", {
        cache: "no-cache",
      })
        .then((res) => res.json())
        .then((json) => {
